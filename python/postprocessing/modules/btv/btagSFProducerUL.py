...
class btagSFProducer(Module):
    ...
    def __init__(
        ...
        self.max_abs_eta = 2.5 #if 'UL' in self.era else 2.4
        ...
        supported_btagSF = {
            'deepjet': {
                ...
                'UL2017': {
                    'inputFileName': "UL2017/btagging.json.gz", 
                    'measurement_types': { # jet_flavour: "measurement_tag"
                        5: "comb",  #0: "comb",  # b
                        4: "comb",  #1: "comb",  # c
                        0: "incl"   #2: "incl"   # light
                ...
                'UL2018': {
                    'inputFileName': "UL2018/btagging.json.gz", 
                    'measurement_types': { # jet_flavour: "measurement_tag"
                        5: "comb",  #0: "comb",  # b
                        4: "comb",  #1: "comb",  # c
                        0: "incl"   #2: "incl"   # light
    ...
    def beginJob(self):
        ...
        inFile_ = os.path.join(self.inputFilePath, self.inputFileName)
        print("self.algo: ", type(self.algo), "  ", self.algo)
        print("inFile_: ", type(inFile_), "  ", inFile_)
        if '.json' not in self.inputFileName:
            raise ValueError("ERROR: btagSFProducerUL is supported for b-tag SFs stored in JSON files only. Input file '%s' is not supported! " % (
                    self.inputFileName))
        else:
            print('btagSFProducerUL:: b-tag algorithm %s, SFs are read from %s' % (
                self.algo, inFile_
            ))
        self.reader = correctionlibcore.CorrectionSet.from_file(inFile_)
    ...
    def getFlavorBTV(self, flavor):
        '''
            ...
            Convert them to the btagging group (pre UltraLegecy) convention of 0, 1, 2
        '''
        (flavor_btv_b, flavor_btv_c, flavor_btv_light) = (5, 4, 0) if 'UL' in self.era else (0, 1, 2)
        flavor_btv = None
        if abs(flavor) == 5:
            flavor_btv = flavor_btv_b
        elif abs(flavor) == 4:
            flavor_btv = flavor_btv_c
        elif abs(flavor) in [0, 1, 2, 3, 21]:
            flavor_btv = flavor_btv_light
        else:
            if self.verbose > 0:
                print(
    ...
    def getWpBTV(self, wp):
        # get working-point name used in BTV-provided SF.json file
        wp_btv = {"l": 0, "m": 1, "t": 2,
    ...
    def getBTagAlgoBTV(self, algo):
        # get b-tag algorithm name used in BTV-provided SF.json file
        algo_btv = {
            'deepjet': 'deepJet',
            'deepcsv': 'deepCSV'
        }.get(algo.lower(), None)
    ...
    def getSFs(self, jet_data, wp_btv, syst, shape_corr=False):
        for idx, (pt, eta, flavor_btv, discr) in enumerate(jet_data):
            ...
            measurement_name = '%s_%s' % (
                self.getBTagAlgoBTV(self.algo),
                'shape' if shape_corr else self.measurement_types[flavor_btv]
            )
            
            # evaluate SF
            sf = None
            if shape_corr:
                sf = self.reader[measurement_name].evaluate(syst, flavor_btv, abs(eta), pt, discr)
            else:
                sf = self.reader[measurement_name].evaluate(syst, wp_btv, flavor_btv, abs(eta), pt)
            #print("pt %g, eta %g, flavor_btv %d measurement_name %s sf %f" % (pt, eta, flavor_btv, measurement_name, sf))
            # check if SF is OK
            if sf < 0.01:
                if self.verbose > 0:
